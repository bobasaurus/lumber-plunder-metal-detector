...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : dac_dma_wave_test main program body
  ******************************************************************************
  */
...
void GenerateWaveToTransmit()
{
	...
	float amplitude = 1000.0f;//[V], and wave will be offset so every value is above zero
	float frequency = 10000;
	float sampleRate = 1000000;//[samp/sec]
...
int main(void)
{
  ...
  GenerateWaveToTransmit();
   ...
   if(HAL_DAC_Start_DMA(&hdac1, DAC_CHANNEL_1, (uint32_t *)waveToTransmit, waveToTransmitSize, DAC_ALIGN_12B_R) != HAL_OK){
