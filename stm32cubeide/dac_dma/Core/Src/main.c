...
const uint32_t waveSampleRate = 5333333;
const double waveFrequency = 10000;
const double waveAmplitude = 2047.5;
const double waveCenter = 2047.5;

#define WAVE_BUFFER_CAPACITY 800	//way bigger than needed
...
int main(void)
{
  ...
  if (GenerateWaveToTransmit(waveSampleRate, waveFrequency, waveAmplitude, waveCenter) != 0)
	  while (1) {}
  ...
  /*
   * Configures the DAC channel for DMA operation.
   * Links the DMA transfer from the memory buffer (dacBuffer) to the DAC data register.
   * Starts both the DAC and the DMA in one call.
   */
  HAL_StatusTypeDef status = HAL_DAC_Start_DMA(&hdac1, DAC_CHANNEL_1, (uint32_t*)waveBuffer, waveSize, DAC_ALIGN_12B_R);



  int test = 1;
  test++;
  //hdma_dac1_ch1.Instance
  //hdma_dac1_ch1.DmaBaseAddress
  //HAL_DMA_Start(&hdma_dac1_ch1, (uint32_t) waveBuffer, (uint32_t)&hdac1.Instance->DHR12R1, (uint32_t) waveSize);
  //__HAL_DAC_ENABLE_DMA(&hdac1, DAC_CHANNEL_1);
